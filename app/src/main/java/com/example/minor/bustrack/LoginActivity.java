                ...
                mAuth.createUserWithEmailAndPassword(email, password).addOnCompleteListener(LoginActivity.this, new OnCompleteListener<AuthResult>() {
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (!task.isSuccessful()) {
                            ...
                            if (isDriver) {
                                DatabaseReference user_db = FirebaseDatabase.getInstance("https://minor-project-e5e07-default-rtdb.firebaseio.com").getReference().child("Users").child("Driver").child(mAuth.getCurrentUser().getUid());
                                user_db.setValue(true);
                            } else {
                                DatabaseReference user_db = FirebaseDatabase.getInstance("https://minor-project-e5e07-default-rtdb.firebaseio.com").getReference().child("Users").child("Students").child(mAuth.getCurrentUser().getUid());
                                user_db.setValue(true);
                            }
                        }
                    }
